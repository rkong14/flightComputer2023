...
void setup() {
  ...
  sdFileWRITE.println(F("FILE BEGIN B.2 1/28/23")); //unclear if this will actually get into the sd card
  sdFileWRITE.println();
  sdFileWRITE.close();
  altSet();
...
void loop() {
  ...
  //heightGained>=70.00
  if (falling >= 3 && !haveDeployed && heightFallen>=1.00){ //if been falling for 1/2s + haven't deployed already + fallen 1m and flown >70m
